...
class SyncDataManager:
    ...
    @classmethod
    def get_account_latest(cls, db_type: str, instance_id: int, 
                          username: str = None, include_deleted: bool = False) -> Optional[CurrentAccountSyncData]:
        """获取账户最新状态"""
    ...
    def get_account_changes(cls, instance_id: int, db_type: str, username: str) -> List[AccountChangeLog]:
        """获取账户变更历史"""
        return AccountChangeLog.query.filter_by(
            instance_id=instance_id, db_type=db_type, username=username
        ).order_by(AccountChangeLog.change_time.desc()).all()
    
    @classmethod
    def upsert_account(cls, instance_id: int, db_type: str, username: str, 
                      permissions_data: dict, is_superuser: bool = False, session_id: str = None) -> CurrentAccountSyncData:
        """根据数据库类型更新账户权限"""
    ...
    @classmethod
    def _upsert_mysql_account(cls, instance_id: int, username: str, 
                             permissions_data: dict, is_superuser: bool, session_id: str = None) -> CurrentAccountSyncData:
        """更新MySQL账户权限"""
    ...
    @classmethod
    def _upsert_postgresql_account(cls, instance_id: int, username: str, 
                                  permissions_data: dict, is_superuser: bool, session_id: str = None) -> CurrentAccountSyncData:
        """更新PostgreSQL账户权限"""
    ...
    @classmethod
    def _upsert_sqlserver_account(cls, instance_id: int, username: str, 
                                 permissions_data: dict, is_superuser: bool, session_id: str = None) -> CurrentAccountSyncData:
        """更新SQL Server账户权限"""
    ...
    @classmethod
    def _upsert_oracle_account(cls, instance_id: int, username: str, 
                              permissions_data: dict, is_superuser: bool, session_id: str = None) -> CurrentAccountSyncData:
    ...
    def _detect_mysql_changes(cls, account: CurrentAccountSyncData, new_permissions: dict) -> dict:
        ...
        if old_db_perms != new_db_perms:
            changes["database_privileges"] = {
                "added": {k: v for k, v in new_db_perms.items() if k not in old_db_perms or set(old_db_perms[k]) != set(v)},
                "removed": {k: v for k, v in old_db_perms.items() if k not in new_db_perms or set(new_db_perms.get(k, [])) != set(v)}
            }
    ...
    def _detect_sqlserver_changes(cls, account: CurrentAccountSyncData, new_permissions: dict) -> dict:
        ...
        if old_db_roles != new_db_roles:
            changes["database_roles"] = {
                "added": {k: v for k, v in new_db_roles.items() if k not in old_db_roles or set(old_db_roles[k]) != set(v)},
                "removed": {k: v for k, v in old_db_roles.items() if k not in new_db_roles or set(new_db_roles.get(k, [])) != set(v)}
            }
    ...
    @classmethod
    def get_accounts_by_instance_and_db_type(cls, instance_id: int, db_type: str, include_deleted: bool = False) -> List[CurrentAccountSyncData]:
        """根据实例ID和数据库类型获取账户列表"""
