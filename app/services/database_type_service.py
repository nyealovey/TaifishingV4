...
class DatabaseTypeService:
    ...
    def get_all_types() -> List[DatabaseTypeConfig]:
        ...
        return DatabaseTypeConfig.query.order_by(
            DatabaseTypeConfig.sort_order, 
            DatabaseTypeConfig.name
        ).all()
    ...
    @staticmethod
    def get_type_by_name(name: str) -> Optional[DatabaseTypeConfig]:
        """根据名称获取数据库类型"""
        return DatabaseTypeConfig.get_by_name(name)
    
    @staticmethod
    def get_type_by_id(type_id: int) -> Optional[DatabaseTypeConfig]:
        """根据ID获取数据库类型"""
