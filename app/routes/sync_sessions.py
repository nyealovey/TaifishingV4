...
from app.utils.structlog_config import get_system_logger, log_info, log_error
...
def api_get_session_detail(session_id):
    ...
    try:
        ...
        log_error(f"获取同步会话详情失败: {str(e)}", module="sync_sessions", user_id=current_user.id, session_id=session_id)
        return jsonify({
            'success': False,
            'message': '获取会话详情失败',
            'error': str(e)
