...
def get_merged_request_logs(query, page=1, per_page=20):
    ...
    try:
                ...
                if match:
                    ...
                    if match:
                        ...
                        if path in merged_requests:
                            ...
                            # 如果没有找到对应的开始日志，记录警告但不创建合并记录
                            logging.warning(f"发现孤立的请求结束日志: 路径={path}, 状态码={status_code}, 日志ID={log.id}")
                            # 不创建合并记录，让这个日志作为普通日志显示
        ...
        for request_id, request_data in merged_requests.items():
            ...
            # 使用开始日志的真实ID
            merged_log = {
                'id': request_data['start_log'].id if request_data['start_log'] else request_data['end_log'].id,
                'level': max_level,
                'log_type': 'request',
