...
def sync_history():
    ...
    try:
        ...
        sync_records = (
            SyncData.query.filter(SyncData.sync_type.in_(["batch", "task"]))
...
def sync_statistics():
    ...
    try:
        ...
        # 总同步次数
        total_syncs = SyncData.query.filter(SyncData.sync_time >= week_ago).count()

        # 成功和失败次数
        success_syncs = SyncData.query.filter(
            SyncData.sync_time >= week_ago, SyncData.status == "success"
        ).count()

        failed_syncs = SyncData.query.filter(
            SyncData.sync_time >= week_ago, SyncData.status == "failed"
        ).count()

        # 总同步账户数
        total_accounts = (
            db.session.query(func.sum(SyncData.synced_count))
            .filter(SyncData.sync_time >= week_ago)
            .scalar()
            or 0
        )
        ...
        # 成功率
        success_rate = round(
            (success_syncs / total_syncs * 100) if total_syncs > 0 else 0, 1
        )

        # 最近同步时间
        last_sync = (
            SyncData.query.filter(SyncData.sync_time >= week_ago)
            .order_by(desc(SyncData.sync_time))
            .first()
        )
        ...
        last_sync_time = (
            format_china_time(last_sync.sync_time, "%Y-%m-%d %H:%M:%S")
            if last_sync
            else None
        )
        ...
        # 数据库类型分布
        db_type_stats = (
            db.session.query(Instance.db_type, func.count(SyncData.id).label("count"))
            .join(SyncData, Instance.id == SyncData.instance_id)
            .filter(SyncData.sync_time >= week_ago)
            .group_by(Instance.db_type)
            .all()
        )

        db_type_distribution = {stat.db_type: stat.count for stat in db_type_stats}
...
def account_statistics():
    ...
    try:
        ...
        active_accounts = base_query.filter(
            Account.is_locked == False, Account.password_expired == False
        ).count()

        # 锁定账户数
        locked_accounts = base_query.filter(Account.is_locked == True).count()

        # 密码过期账户数
        expired_accounts = base_query.filter(Account.password_expired == True).count()
        ...
        # 按数据库类型统计账户数
        db_type_stats = (
            db.session.query(Instance.db_type, func.count(Account.id).label("count"))
            .join(Account, Instance.id == Account.instance_id)
            .group_by(Instance.db_type)
            .all()
        )
...
def sync_records():
    ...
    if date_range:
        ...
        if date_range == "today":
            ...
            query = query.filter(
                SyncData.sync_time >= today_start_utc,
                SyncData.sync_time <= today_end_utc,
            )
    ...
    class Pagination:
        ...
        def iter_pages(
            self, left_edge=2, right_edge=2, left_current=2, right_current=3
        ):
...
def export():
    ...
    if format_type == "csv":
        ...
        return Response(
            ...
            mimetype="text/csv",
            headers={
                "Content-Disposition": "attachment; filename=account_sync_history.csv"
            },
        )
    ...
    try:
        ...
        for db_type in ["mysql", "postgresql", "oracle", "sqlserver"]:
            accounts = (
                ...
                .filter(Instance.db_type == db_type)
                .all()
            )
        ...
        for env in ["production", "development", "testing"]:
            accounts = (
                db.session.query(Account)
                .join(Instance, Account.instance_id == Instance.id)
                .filter(Instance.environment == env)
                .all()
            )
        ...
        # 最近账户
        recent_accounts = (
            Account.query.order_by(Account.created_at.desc()).limit(10).all()
        )
...
def get_filter_options():
    ...
    try:
        ...
        classifications = (
            ...
            .order_by(AccountClassification.priority.desc(), AccountClassification.name)
            .all()
        )
    ...
    try:
        ...
        for classification in classifications:
            account_count = (
                ...
                .filter(AccountClassification.classification_id == classification.id)
                .count()
            )
        ...
        return jsonify(
            {
                ...
                "account": {
                    ...
                    "host": account.host,
                    "instance": {"db_type": instance.db_type, "name": instance.name},
                },
