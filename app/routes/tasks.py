...
def create():
    ...
    if request.is_json:
        return jsonify({"instances": [instance.to_dict() for instance in instances]})
    ...
    for db_type in db_types:
        try:
            ...
            existing_task = Task.query.filter_by(
                name=task_name, task_type=task_type, db_type=db_type
            ).first()
    ...
    try:
        ...
        if task.task_type == "sync_accounts":
            ...
            query = SyncData.query.filter(
                SyncData.sync_type == "task", SyncData.task_id == task_id
        ...
        for execution in executions.items:
            execution_list.append(
                {
                    "id": execution.id,
                    "sync_time": format_china_time(
                        execution.sync_time, "%Y-%m-%d %H:%M:%S"
                    ),
                    ...
                    "instance_name": (
                        execution.instance.name if execution.instance else "未知实例"
                    ),
                }
            )
        ...
        return (
            jsonify({"success": False, "error": f"获取任务执行记录失败: {str(e)}"}),
            500,
        )
...
def task_execution_detail(task_id, execution_id):
    ...
    try:
        ...
        return jsonify({"success": True, "execution": execution_detail})
    except Exception as e:
        logging.error(f"获取任务执行详情失败: {e}")
        return (
            jsonify({"success": False, "error": f"获取任务执行详情失败: {str(e)}"}),
            500,
        )
...
def edit(task_id):
    ...
    return render_template("tasks/edit.html", task=task, instances=instances)
...
def run_now(task_id):
    ...
    try:
        ...
        if request.is_json:
            return jsonify({"message": "任务已提交执行", "task_id": result.id})
...
def api_run_now(task_id):
    ...
    try:
        result = execute_task.delay(task.id)
        return jsonify(
            {"success": True, "message": "任务已提交执行", "task_id": result.id}
        )
    except Exception as e:
