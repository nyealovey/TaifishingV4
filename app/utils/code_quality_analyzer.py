...
class CodeQualityAnalyzer:
    ...
    def _calculate_maintainability_index(self, lines_of_code: int, cyclomatic_complexity: int, comment_ratio: float) -> float:
        """计算可维护性指数 (0-100)"""
    ...
    def _generate_quality_report(self) -> Dict[str, Any]:
        ...
        avg_complexity = sum(metrics.cyclomatic_complexity for metrics in self.metrics.values()) / total_files if total_files > 0 else 0
        avg_maintainability = sum(metrics.maintainability_index for metrics in self.metrics.values()) / total_files if total_files > 0 else 0
