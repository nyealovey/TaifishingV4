...
class APIResponse:
    ...
    def validation_error(
        errors: Dict[str, List[str]], message: str = "数据验证失败"
    ) -> Dict[str, Any]:
        """
        验证错误响应
    ...
    @staticmethod
    def not_found(resource: str = "资源", message: str = None) -> Dict[str, Any]:
        """
        资源未找到响应
        ...
        return APIResponse.error(message=message, code=404, error_code="NOT_FOUND")
    ...
    def unauthorized(message: str = "未授权访问") -> Dict[str, Any]:
        ...
        Returns:
            dict: 401响应
        """
        return APIResponse.error(message=message, code=401, error_code="UNAUTHORIZED")
    ...
    def forbidden(message: str = "禁止访问") -> Dict[str, Any]:
        ...
        Returns:
            dict: 403响应
        """
        return APIResponse.error(message=message, code=403, error_code="FORBIDDEN")

    @staticmethod
    def rate_limited(
        message: str = "请求过于频繁", retry_after: int = 60
    ) -> Dict[str, Any]:
        """
        速率限制响应
    ...
    @staticmethod
    def server_error(
        message: str = "服务器内部错误", error_id: str = None
    ) -> Dict[str, Any]:
        """
        服务器错误响应
...
def paginated_response(
    page_param: str = "page", per_page_param: str = "per_page", max_per_page: int = 100
):
    """分页响应装饰器"""
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if isinstance(result, list):
                    ...
                    return APIResponse.paginated(
                        data=paginated_data, page=page, per_page=per_page, total=total
                    )
...
def log_request(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            duration = (now() - start_time).total_seconds()
            logger.error(
                f"API请求失败: {func.__name__}, 耗时: {duration:.3f}秒, 错误: {e}"
            )
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if duration > threshold:
                    logger.warning(
                        f"慢API响应: {func.__name__}, 耗时: {duration:.3f}秒"
                    )
                else:
