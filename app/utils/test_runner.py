...
class TestRunner:
    ...
    def _run_unit_tests(self) -> Dict[str, Any]:
        ...
        for test in tests:
            try:
                ...
                failed += 1
                results.append(
                    {"name": test.__name__, "success": False, "error": str(e)}
                )
    ...
    def _test_credential_creation(self) -> Dict[str, Any]:
        ...
        try:
            credential = Credential(
                name="test_credential", username="test_user", password="test_password"
            )
            db.session.add(credential)
            db.session.commit()

            # 验证凭据创建
            created_credential = Credential.query.filter_by(
    ...
    def _test_validation_functions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "name": "test_validation_functions",
                "success": False,
                "error": str(e),
            }
    ...
    def _test_authorization_checks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "name": "test_authorization_checks",
                "success": False,
                "error": str(e),
            }
    ...
    def test_decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                execution_time = time.time() - start_time
                logger.error(
                    f"测试失败: {test_name} (耗时: {execution_time:.3f}秒) - {e}"
                )
                return {
                    "name": test_name,
                    "success": False,
    ...
    def test_decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if execution_time > threshold:
                logger.warning(
                    f"性能测试失败: {func.__name__} 耗时 {execution_time:.3f}秒 (阈值: {threshold}秒)"
                )
                return {
                    "name": func.__name__,
                    "success": False,
