...
class ClassificationBatch(db.Model):
    ...
    # 批次类型
    batch_type = Column(String(20), nullable=False, comment='批次类型: manual(手动), scheduled(定时), api(API)')
    
    # 批次状态
    status = Column(String(20), nullable=False, default='running', comment='批次状态: running(运行中), completed(完成), failed(失败)')
