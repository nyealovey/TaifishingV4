...
class PermissionConfig(db.Model):
    ...
    def init_default_permissions(cls):
        ...
        oracle_permissions = [
            ...
            ('oracle', 'roles', 'LOGSTDBY_ADMINISTRATOR', '逻辑备用数据库管理员角色', 17),
            ('oracle', 'roles', 'DBFS_ROLE', '数据库文件系统角色', 18),
            ('oracle', 'roles', 'XDBADMIN', 'XML数据库管理员角色', 19),
            ('oracle', 'roles', 'XDBWEBSERVICES', 'XML数据库Web服务角色', 20),
            ('oracle', 'roles', 'XDB_WEBSERVICES_OVER_HTTP', 'HTTP上的XML数据库Web服务角色', 21),
            ('oracle', 'roles', 'XDB_WEBSERVICES_WITH_PUBLIC', '公共XML数据库Web服务角色', 22),
            ('oracle', 'roles', 'XDB_WEBSERVICES_WITH_CREDENTIAL', '凭据XML数据库Web服务角色', 23),
            ('oracle', 'roles', 'XDB_WEBSERVICES_WITH_CREDENTIAL_AND_ACL', '凭据和ACL的XML数据库Web服务角色', 24),
            ('oracle', 'roles', 'XDB_WEBSERVICES_WITH_ACL', 'ACL的XML数据库Web服务角色', 25),
            ('oracle', 'roles', 'XDB_WEBSERVICES_WITH_CREDENTIAL_AND_ACL_AND_PUBLIC', '凭据、ACL和公共的XML数据库Web服务角色', 26),
            ('oracle', 'roles', 'XDB_WEBSERVICES_WITH_CREDENTIAL_AND_ACL_AND_PUBLIC_AND_HTTP', '凭据、ACL、公共和HTTP的XML数据库Web服务角色', 27),
            ('oracle', 'roles', 'XDB_WEBSERVICES_WITH_CREDENTIAL_AND_ACL_AND_PUBLIC_AND_HTTP_AND_HTTPS', '凭据、ACL、公共、HTTP和HTTPS的XML数据库Web服务角色', 28),
            ('oracle', 'roles', 'XDB_WEBSERVICES_WITH_CREDENTIAL_AND_ACL_AND_PUBLIC_AND_HTTP_AND_HTTPS_AND_FTP', '凭据、ACL、公共、HTTP、HTTPS和FTP的XML数据库Web服务角色', 29),
            ('oracle', 'roles', 'XDB_WEBSERVICES_WITH_CREDENTIAL_AND_ACL_AND_PUBLIC_AND_HTTP_AND_HTTPS_AND_FTP_AND_FTPS', '凭据、ACL、公共、HTTP、HTTPS、FTP和FTPS的XML数据库Web服务角色', 30),
            
            # 表空间权限
            ('oracle', 'tablespace_privileges', 'CREATE TABLESPACE', '创建表空间权限', 1),
            ('oracle', 'tablespace_privileges', 'ALTER TABLESPACE', '修改表空间权限', 2),
            ('oracle', 'tablespace_privileges', 'DROP TABLESPACE', '删除表空间权限', 3),
            ('oracle', 'tablespace_privileges', 'MANAGE TABLESPACE', '管理表空间权限', 4),
            ('oracle', 'tablespace_privileges', 'UNLIMITED TABLESPACE', '无限制表空间权限', 5),
            ('oracle', 'tablespace_privileges', 'CREATE ANY TABLESPACE', '创建任意表空间权限', 6),
            ('oracle', 'tablespace_privileges', 'ALTER ANY TABLESPACE', '修改任意表空间权限', 7),
            ('oracle', 'tablespace_privileges', 'DROP ANY TABLESPACE', '删除任意表空间权限', 8),
            ('oracle', 'tablespace_privileges', 'MANAGE ANY TABLESPACE', '管理任意表空间权限', 9),
            ('oracle', 'tablespace_privileges', 'SELECT ANY TABLESPACE', '查询任意表空间权限', 10),
            ('oracle', 'tablespace_privileges', 'INSERT ANY TABLESPACE', '插入任意表空间权限', 11),
            ('oracle', 'tablespace_privileges', 'UPDATE ANY TABLESPACE', '更新任意表空间权限', 12),
            ('oracle', 'tablespace_privileges', 'DELETE ANY TABLESPACE', '删除任意表空间权限', 13),
            ('oracle', 'tablespace_privileges', 'EXECUTE ANY TABLESPACE', '执行任意表空间权限', 14),
            ('oracle', 'tablespace_privileges', 'DEBUG ANY TABLESPACE', '调试任意表空间权限', 15),
            ('oracle', 'tablespace_privileges', 'FLASHBACK ANY TABLESPACE', '闪回任意表空间权限', 16),
            ('oracle', 'tablespace_privileges', 'ON COMMIT REFRESH ANY TABLESPACE', '提交时刷新任意表空间权限', 17),
            ('oracle', 'tablespace_privileges', 'QUERY REWRITE ANY TABLESPACE', '查询重写任意表空间权限', 18),
            ('oracle', 'tablespace_privileges', 'UNDER ANY TABLESPACE', '继承任意表空间权限', 19),
            ('oracle', 'tablespace_privileges', 'WRITE ANY TABLESPACE', '写入任意表空间权限', 20),
            ('oracle', 'tablespace_privileges', 'READ ANY TABLESPACE', '读取任意表空间权限', 21),
