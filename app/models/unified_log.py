...
class UnifiedLog(db.Model):
    ...
    @classmethod
    def get_recent_logs(
        cls, hours: int = 24, level: LogLevel | None = None, module: str | None = None, limit: int = 100
    ) -> list["UnifiedLog"]:
        """获取最近的日志"""
    ...
    def get_logs_by_module(cls, module: str, hours: int = 24, limit: int = 100) -> list["UnifiedLog"]:
        """根据模块获取日志"""
        return (
            cls.query.filter(cls.module == module, cls.timestamp >= datetime.utcnow() - timedelta(hours=hours))
            .order_by(cls.timestamp.desc())
            .limit(limit)
            .all()
        )
    ...
    def get_log_statistics(cls, hours: int = 24) -> dict[str, Any]:
        ...
        error_count = cls.query.filter(
            cls.timestamp >= start_time, cls.level.in_([LogLevel.ERROR, LogLevel.CRITICAL])
        ).count()
