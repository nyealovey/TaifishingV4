...
class AccountClassification(db.Model):
    ...
    # 关联关系
    rules = db.relationship('ClassificationRule', backref='classification', lazy='dynamic', cascade='all, delete-orphan')
    account_assignments = db.relationship('AccountClassificationAssignment', backref='classification', lazy='dynamic', cascade='all, delete-orphan')
...
class AccountClassificationAssignment(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('account_id', 'classification_id', name='unique_account_classification'),
    )
