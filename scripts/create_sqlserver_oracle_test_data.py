...
from app.models import Credential, Instance, db
...
def create_test_data():
    ...
    try:
        ...
        with app.app_context():
            ...
            # 检查是否已存在
            existing_sqlserver = Instance.query.filter_by(name=sqlserver_instance.name, db_type="sqlserver").first()
            ...
            existing_sqlserver_cred = Credential.query.filter_by(
                instance_id=sqlserver_instance.id, username="sa"
            ).first()
            ...
            # 检查是否已存在
            existing_oracle = Instance.query.filter_by(name=oracle_instance.name, db_type="oracle").first()
            ...
            # 检查是否已存在
            existing_oracle_cred = Credential.query.filter_by(instance_id=oracle_instance.id, username="system").first()
            ...
            # 显示创建的实例
            print("\n📋 创建的实例列表:")
            instances = Instance.query.filter(Instance.name.in_(["SQL Server测试实例", "Oracle测试实例"])).all()
