...
def main():
    ...
    # 1. Black 格式化检查
    success, output = run_command(["uv", "run", "black", "app/", "--check", "--diff"], "Black 代码格式化检查")
    results.append(("Black", success))

    # 2. isort 导入排序检查
    success, output = run_command(["uv", "run", "isort", "app/", "--check-only", "--diff"], "isort 导入排序检查")
    results.append(("isort", success))

    # 3. Ruff 代码检查
    success, output = run_command(["uv", "run", "ruff", "check", "app/"], "Ruff 代码检查")
    results.append(("Ruff", success))

    # 4. Mypy 类型检查
    success, output = run_command(["uv", "run", "mypy", "app/"], "Mypy 类型检查")
